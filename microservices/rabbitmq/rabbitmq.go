...
// When publisher confirms are enabled, serializes through pubMu to prevent orphan
// confirmations from confusing publishAndWaitConfirm's confirm-wait sequence.
func (c Conn) PublishMessage(exchangeName, routingKey string, message []byte) error {
	...
	// Drain the confirmation generated by this publish so it doesn't
	// accumulate and confuse a subsequent PublishReliable call.
	if c.confirmCh != nil {
		select {
		case <-c.confirmCh:
		case <-time.After(2 * time.Second):
			log.Warn("PublishMessage: timed out draining confirm (non-fatal)")
		}
	}
...
//
// A generation counter prevents the "thundering herd" reconnect storm: when N
// goroutines all fail on the same dead connection, only the first one reconnects;
// the rest see that the generation has advanced and reuse the fresh connection.
type ConnManager struct {
	mu         sync.RWMutex
	conn       Conn
	conf       config.RabbitMQ
	generation uint64 // incremented on every reconnect; prevents thundering herd
}
...
// connSnapshot returns the current connection and its generation under read lock.
// Callers use the returned generation to avoid thundering-herd reconnects.
func (m *ConnManager) connSnapshot() (Conn, uint64) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.conn, m.generation
}

// reconnectIfStale acquires the write lock and reconnects only if no other
// goroutine has already done so (i.e. generation hasn't advanced since the
// caller's snapshot). Returns the current conn after the operation.
func (m *ConnManager) reconnectIfStale(oldGen uint64) Conn {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.generation != oldGen {
		// Another goroutine already reconnected — reuse its connection.
		log.Debug("ConnManager: skipping reconnect, another goroutine already refreshed the connection")
		return m.conn
	}
	m.reconnectLocked()
	return m.conn
...
// Uses generation-gated reconnect to prevent thundering herd when many goroutines
// fail simultaneously on the same dead connection.
func (m *ConnManager) PublishReliable(exchange, routingKey string, message []byte, maxRetries int) error {
	conn, gen := m.connSnapshot()
	...
	log.Warnf("ConnManager.PublishReliable: failed (gen=%d), reconnecting if stale: %v", gen, err)
	conn = m.reconnectIfStale(gen)
...
// Uses generation-gated reconnect to prevent thundering herd.
func (m *ConnManager) PublishMessage(exchange, routingKey string, message []byte) error {
	...
	log.Warnf("ConnManager.PublishMessage: failed (gen=%d), reconnecting if stale: %v", gen, err)
	conn = m.reconnectIfStale(gen)
