...
func (es *elasticsearchStorage) GetAllPublishedBlogsMetadata(ctx context.Context, limit, offset int) ([]map[string]interface{}, error) {
	// Build the query to get all published blogs with sorting by published_time or blog.time as fallback
	query := map[string]interface{}{
		"sort": []map[string]interface{}{
			{
	...
	es.log.Infof("GetAllPublishedBlogsMetadata: successfully fetched %d blogs metadata", len(blogsMetadata))
	return blogsMetadata, nil
}
